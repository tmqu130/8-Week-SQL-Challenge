...
select 
    (datediff(week, '2021-01-01', registration_date) + 1) as week, 
    count(*) as regis_runner
...
with CTE as 
    (select 
        order_id, 
        count(pizza_id) as pizza_number
    from pizza_runner.customer_orders
    group by order_id)
select 
    pizza_number, 
    avg(datediff(minute, order_date, pickup_time)) as avg_arrive_time
from CTE  
...
select 
    customer_id, 
    round(avg(distance), 1) as avg_customer_distance
...
select 
    ...
    order_id, 
    distance, 
    round((cast(duration as float)/60), 1) as duration, 
    round((distance/(cast(duration as float)/60)), 1)  as avg_speed
from pizza_runner.runner_orders
where cancellation = 'No'
order by 
    runner_id, 
    (distance/(cast(duration as float)/60));

--7. What is the successful delivery percentage for each runner?

select 
    runner_id,  
    (   cast(count(case when cancellation = 'No' then 1 end) as float)
        /
        cast(count(*) as float) 
    ) as successful_rate
from pizza_runner.runner_orders
